...
namespace Programs
{
    ...
    public static class ShellCommands
    {
        ...
        private static void Shell_OnConnected(ShellCore Shell, string RemoteAddress, DateTime Time)
        {
            ...
            if (Terminal_Server.Program.lxiMode == false)
            {
        ...
        private static void Shell_OnCommandReceived(ShellCore Shell, string[] Arguments, ref bool SuspressError, DateTime Time)
        {
            switch (Arguments[0].ToUpper())
            {
                ...
                case "HELP":
                    ...
                    break;
                case "*IDN?":           //added by 7m4mon
                    Shell.TelnetServer.Print(Tools.HardwareProvider + ",", true);
                    Shell.TelnetServer.Print("Lxiduino" + ",", true);     //i don't know how to get assambly name.
                    Shell.TelnetServer.Print(SystemInfo.Version.ToString()  + "," ,true);
                    Shell.TelnetServer.Print("7m4mon");                        //infuture, this string change to mac address.
                    SuspressError = true;
                    break;
            }
        }
        ...
        private static bool DoHelp(TelnetServer Server, string Page)
        {
            switch (Page)
            {
                ...
                case "QUIT":
                    ...
                    return true;
                case "*IDN?":
                    Server.Print("*IDN?                              Returns the idendfy string");
                    return true;
                default:
                    return false;
            }
